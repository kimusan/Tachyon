...
//				\MailSo\Base\Utils::mkdir($sNewDir)
	 ...
	 */
	public static function ConvertInsecureAccounts(\Tachyon\Actions $oActions, \Tachyon\Model\MainAccount $oMainAccount) : array
	{
		...
		if (1 < \count($aAccounts)) {
			...
			foreach ($aAccounts as $sEmail => $sToken) {
				...
				try {
					...
					if (!$sToken) {
						\Tachyon\Util\Log::warning('UPGRADE', "ConvertInsecureAccount {$sEmail} no token");
						continue;
					}
					...
					if (empty($aAccountHash[0]) || 'token' !== $aAccountHash[0] // simple token validation
						...
						\Tachyon\Util\Log::warning('UPGRADE', "ConvertInsecureAccount {$sEmail} invalid aAccountHash: " . \print_r($aAccountHash,1));
						continue;
					}
					...
					\Tachyon\Util\Log::warning('UPGRADE', "ConvertInsecureAccount {$sEmail} failed");
				}
			}
	 ...
	 * Attempt to convert the old less secure data into better secured data
	 */
	public static function ConvertInsecureContactsSync(\Tachyon\Actions $oActions, \Tachyon\Model\Account $oAccount) : ?array
	{
...
//		$tar_destination = APP_DATA_FOLDER_PATH . APP_VERSION . '.tar';
		...
		if (\class_exists('PharData')) {
			...
			$tar_destination .= '.tgz';
			$tar = new \Tachyon\Util\Stream\TAR($tar_destination);
		}
		...
		if ($tar instanceof \Tachyon\Util\Stream\TAR) {
