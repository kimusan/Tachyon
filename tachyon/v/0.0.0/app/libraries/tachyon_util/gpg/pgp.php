...
namespace Tachyon\Util\GPG;

use Tachyon\Util\SensitiveString;

class PGP extends Base implements \Tachyon\Util\PGP\PGPInterface
{
	private
		$_message,
...
//		\rewind($fp);
	...
	{
//		\Tachyon\Util\Log::debug('GnuPG', "export({$fingerprint}, {$passphrase})");
...
//			'NO_SECKEY' => [],
	...
	private function exec(array $arguments, bool $throw = true) /*: array|false*/
	{
		if (\version_compare($this->version, '2.2.5', '<')) {
			\Tachyon\Util\Log::error('GPG', "{$this->version} too old");
			return false;
		}
