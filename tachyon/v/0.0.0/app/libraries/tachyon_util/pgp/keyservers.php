...
*/
	...
	private static function fetch(string $host, string $op, string $search, bool $fingerprint = false, bool $exact = false) : ?\Tachyon\Util\HTTP\Response
	{
		...
		\Tachyon\Util\Log::debug('PGP', $url);
		return static::HTTP()->doRequest('GET', $url);
	}

	private static $HTTP;
	private static function HTTP() : \Tachyon\Util\HTTP\Request
	{
		if (!static::$HTTP) {
			static::$HTTP = \Tachyon\Util\HTTP\Request::factory(/*'socket' or 'curl'*/);
...
*/
		foreach (static::$hosts as $host) {
			...
			if (!$oResponse) {
				\Tachyon\Util\Log::info('PGP', "No response for key {$keyId} on {$host}");
				continue;
			}
			if (200 !== $oResponse->status) {
				\Tachyon\Util\Log::debug('PGP', "{$oResponse->status} for key {$keyId} on {$host}");
				continue;
			}
	...
	public static function index(string $search, bool $fingerprint = true, bool $exact = false) : array
	{
		...
		foreach (static::$hosts as $host) {
			...
			if (200 !== $oResponse->status) {
				\Tachyon\Util\Log::debug('PGP', "{$oResponse->status} for search `{$search}` on {$host}");
				continue;
			}
			...
			foreach ($result as $line) {
				...
				if ('pub' === $line[0]) {
					...
					// Ignore invalid line
					if (7 !== \count($line)) {
						\Tachyon\Util\Log::info('PGP', "Invalid pub line for search `{$search}` on {$host}");
						continue;
					}
...
//						'expired'  => \str_contains($line[6], 'e'),
				...
				// uid:<escaped uid string>:<creationdate>:<expirationdate>:<flags>
					...
					// Ignore invalid line
					if (5 !== \count($line)) {
						\Tachyon\Util\Log::info('PGP', "Invalid uid line for search `{$search}` on {$host}");
						continue;
					}
					// Ignore flagged or expired key
