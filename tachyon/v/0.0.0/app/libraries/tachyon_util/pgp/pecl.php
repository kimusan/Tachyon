...
class PECL implements \Tachyon\Util\PGP\PGPInterface
{
	private
		$homedir,
		// Instance of gnupg pecl extension https://www.php.net/gnupg
		$GnuPG,
		// Instance of \Tachyon\Util\GPG\PGP
		$GPG;
...
*/
	...
	{
//		\Tachyon\Util\Log::debug('GnuPG', "addDecryptKey({$fingerprint}, {$passphrase})");
	...
	{
//		\Tachyon\Util\Log::debug('GnuPG', "addSignKey({$fingerprint}, {$passphrase})");
...
//		\rewind($fp);
	...
	public function verify(string $signed_text, string $signature, ?string &$plaintext = null) /*: array|false*/
	{
		...
		if (!$result) {
			\Tachyon\Util\Log::notice('GnuPG', 'gnupg_verify() failed: ' . $this->GnuPG->geterror());
			\Tachyon\Util\Log::info('GnuPG', \print_r($this->GnuPG->geterrorinfo(),1));
		}
		return $result;
	}
