...
//				\ini_set('display_errors', '1');
	...
	public static function getCSP(?string $sScriptNonce = null) : \Tachyon\Util\HTTP\CSP
	{
		...
		$CSP->report_only = $oConfig->Get('debug', 'enable', false); // || TACHYON_DEV
	...
	public static function Logger() : \MailSo\Log\Logger
	{
		...
		if (!$oLogger) {
			...
			if ($oConfig->Get('debug', 'enable', false)) {
				...
				$oLogger->SetLevel(\max(3, \Tachyon\Api::Config()->Get('logs', 'level', \LOG_WARNING)));
			}
		}
		return $oLogger;
	}
	...
	) : ?string
	{
		...
		return static::Actions()->Cacher()->Set(
			...
			\Tachyon\Util\Crypt::EncryptToJSON(array(
				'Email' => $sEmail,
				'Password' => $sPassword,
