...
class Manager
{
	...
	private \Tachyon\Actions $oActions;
	...
	private static function registerLegacyAliases() : void
	{
		...
		foreach ([
			...
			'Tachyon\\Providers\\Storage'             => 'RainLoop\\Providers\\Storage',
			'Tachyon\\Providers\\Files'               => 'RainLoop\\Providers\\Files',
			'Tachyon\\Providers\\Domain'              => 'RainLoop\\Providers\\Domain',
			'Tachyon\\Providers\\AddressBook'         => 'RainLoop\\Providers\\AddressBook',
			'Tachyon\\Providers\\Filters'             => 'RainLoop\\Providers\\Filters',
			'Tachyon\\Providers\\Identities'          => 'RainLoop\\Providers\\Identities',
			'Tachyon\\Providers\\Settings'            => 'RainLoop\\Providers\\Settings',
			'Tachyon\\Providers\\Suggestions'         => 'RainLoop\\Providers\\Suggestions',
	...
	public function CreatePluginByName(string $sName) : ?\Tachyon\Plugins\AbstractPlugin
	{
		$oPlugin = null;
		...
		if ($sClassName) {
			...
			$oPlugin
				...
				->SetPluginConfig(new \Tachyon\Config\Plugin($sName, $oPlugin->ConfigMap(true)))
			;
	...
	public function loadPluginByName(string $sName) : ?string
	{
		if (\preg_match('/^[a-z0-9\\-]+$/', $sName)) {
			...
			if (\class_exists($sClassName) && \is_subclass_of($sClassName, 'Tachyon\\Plugins\\AbstractPlugin')) {
				return $sClassName;
			}
	...
	public function Actions() : \Tachyon\Actions
	{
	...
	public function InitAppData(bool $bAdmin, array &$aAppData, ?\Tachyon\Model\Account $oAccount = null) : self
	{
		if ($this->bIsEnabled && isset($aAppData['Plugins']) && \is_array($aAppData['Plugins'])) {
			...
			foreach ($this->aPlugins as $oPlugin) {
				if ($oPlugin) {
					...
					if (\is_array($aMap)) {
						foreach ($aMap as /* @var $oPluginProperty \Tachyon\Plugins\Property */ $oPluginProperty) {
