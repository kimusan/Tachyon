...
//		DEPRECATED = '',
	...
	final public function Config() : \Tachyon\Config\Plugin
	{
		return $this->oPluginConfig;
	}

	final public function Manager() : \Tachyon\Plugins\Manager
	{
	...
	final public function ConfigMap(bool $flatten = false) : array
	{
		...
		if ($flatten) {
			...
			foreach ($this->aConfigMap as $oItem) {
				if ($oItem) {
					if ($oItem instanceof \Tachyon\Plugins\Property) {
						$result[] = $oItem;
					} else if ($oItem instanceof \Tachyon\Plugins\PropertyCollection) {
						foreach ($oItem as $oSubItem) {
							if ($oSubItem && $oSubItem instanceof \Tachyon\Plugins\Property) {
								$result[] = $oSubItem;
							}
						}
					}
	...
	final public function SetPluginManager(\Tachyon\Plugins\Manager $oPluginManager) : self
	{
	...
	final public function SetPluginConfig(\Tachyon\Config\Plugin $oPluginConfig) : self
	{
