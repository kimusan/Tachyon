...
abstract class Autoconfig
{
	...
	{
//		$emailaddress = \Tachyon\Util\IDN::emailToAscii($emailaddress);
		$domain = \MailSo\Base\Utils::getEmailAddressDomain($emailaddress);
//		$domain = \Tachyon\Util\IDN::toAscii($domain);
...
//			?: static::mx($domain, $emailaddress)
	...
	private static function mx(string $domain, string $emailaddress) : ?array
	{
		...
		foreach (\Tachyon\Util\DNS::MX($domain) as $hostname) {
	...
	{
		$srv = \Tachyon\Util\DNS::SRV('_submissions._tcp.'.$domain);
		if (empty($srv[0]['target'])) {
			$srv = \Tachyon\Util\DNS::SRV('_submission._tcp.'.$domain);
		}
		if (!empty($srv[0]['target'])) {
			$result = [
				'incomingServer' => [],
			...
			$srv = \Tachyon\Util\DNS::SRV('_imaps._tcp.'.$domain);
			if (empty($srv[0]['target'])) {
				$srv = \Tachyon\Util\DNS::SRV('_imap._tcp.'.$domain);
			}
			if (!empty($srv[0]['target'])) {
	...
	{
		foreach (\Tachyon\Util\DNS::SRV("_autodiscover._tcp.{$domain}") as $record) {
			if (443 == $record['port']) {
