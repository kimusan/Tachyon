...
class DefaultDomain implements DomainInterface
{
	...
	{
//		return ('*' === $sName) ? 'default' : \str_replace('*', '_wildcard_', \Tachyon\Util\IDN::toAscii($sName));
...
//		$aList = \glob($this->sDomainPath.'/*.{ini,json,alias}', GLOB_BRACE);
	...
	{
//		$sName = \Tachyon\Util\IDN::toAscii($sName);
		...
		if (\file_exists($sRealFileBase.'.json')) {
			...
			return \Tachyon\Model\Domain::fromArray($sName, $aDomain);
		}

		if (\file_exists($sRealFileBase.'.ini')) {
			$aDomain = \parse_ini_file($sRealFileBase.'.ini') ?: array();
			return \Tachyon\Model\Domain::fromIniArray($sName, $aDomain);
		}
		...
		if ($bFindWithWildCard) {
			...
			$sFoundValue = '';
			if (\strlen($sNames)
			 && \Tachyon\Plugins\Helper::ValidateWildcardValues($sName, $sNames, $sFoundValue)
			 && \strlen($sFoundValue)
			) {
	...
	public function Save(\Tachyon\Model\Domain $oDomain) : bool
	{
		...
		\Tachyon\Utils::saveFile("{$this->sDomainPath}/{$sRealFileName}.json", \json_encode($oDomain, \JSON_PRETTY_PRINT));
	...
	{
//		$sTarget = \Tachyon\Util\IDN::toAscii($sTarget);
...
//		$this->Delete($sName);
		\Tachyon\Utils::saveFile("{$this->sDomainPath}/{$sRealFileName}.alias", $sTarget);
		$this->oCacher && $this->oCacher->Delete(static::CACHE_KEY);
		return true;
	}
	...
	protected function getDisabled() : array
	{
		...
		$aDisabled = array();
		// Tachyon use comma, we use newline
		$sItem = \strtok($sFile, ",\n");
		while (false !== $sItem) {
//			$aDisabled[] = \Tachyon\Util\IDN::toAscii($sItem);
...
//		$sName = \Tachyon\Util\IDN::toAscii($sName);
		...
		if ($sName) {
			...
			\Tachyon\Utils::saveFile($this->sDomainPath.'/disabled', \implode("\n", $aResult));
