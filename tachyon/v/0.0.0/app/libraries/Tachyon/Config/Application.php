...
namespace Tachyon\Config;

class Application extends \Tachyon\Config\AbstractConfig
{
...
; Please don\'t add custom parameters here, those will be overwritten',
	...
	public function Load() : bool
	{
		...
		if (!$sCipher || !\Tachyon\Util\Crypt::cipherSupported($sCipher)) {
			$sCipher && \Tachyon\Util\Log::warning('Crypt', "OpenSSL no support for cipher '{$sCipher}'");
			$aCiphers = \Tachyon\Util\Crypt::listCiphers();
	...
	public function Set(string $sSectionKey, string $sParamKey, $mParamValue) : void
	{
		...
		if ('language' === $sParamKey) {
			$mParamValue = \Tachyon\Util\L10n::validLanguage($mParamValue, 'admin_panel' === $sSectionKey) ?: 'en';
		}
		parent::Set($sSectionKey, $sParamKey, $mParamValue);
	}

	public function SetPassword(\Tachyon\Util\SensitiveString $oPassword) : void
	{
		$this->Set('security', 'admin_password', \password_hash($oPassword, PASSWORD_DEFAULT));
	}

	public function ValidatePassword(\Tachyon\Util\SensitiveString $oPassword) : bool
	{
...
'),
				...
				'sign_me_auto' => array(\Tachyon\Enumerations\SignMeType::DefaultOff->value,
...
Enables caching in the system'),
			...
			'imap' => array(
				...
				'search_all_folders' => array(true, 'Allow searching all folders in the active account'),
				'search_subtree_fallback' => array(true, 'Search subfolders on servers without MULTISEARCH by searching each folder in turn (one IMAP search per folder)'),
			),
