...
use Tachyon\Enumerations\SignMeType;
...
class Actions
{
	 ...
	 * to decrypt a \Tachyon\Model\Account which is stored at
	...
	/**
	 * This session cookie optionally contains a \Tachyon\Model\AdditionalAccount
	 * Value is Base64 EncryptToJSON
	 */
	...
	/**
	 * @var \Tachyon\Providers\Files
	 */
	protected $oFilesProvider = null;

	/**
	 * @var \Tachyon\Providers\Domain
	 */
	protected $oDomainProvider = null;

	/**
	 * @var \Tachyon\Providers\Settings
	 */
...
/*
		try {
			if (!\Tachyon\Util\Shutdown::count() && $this->ImapClient()->IsLoggined()) {
	...
	public function AppData(bool $bAdmin): array
	{
		...
		$aResult = array(
			...
			'System' => array(
				...
				'languages' => \Tachyon\Util\L10n::getLanguages(false),
				'webPath' => \Tachyon\Utils::WebPath(),
				'webVersionPath' => \Tachyon\Utils::WebVersionPath()
			),
		...
		if ($bAdmin) {
			...
			if ($oAccount) {
				...
				$aResult['contactsBccLimit'] = \max(1,
					(int) $oConfig->Get('contacts', 'bcc_recommend_limit', 20));
				...
				if ($aResult['calendarAllowed'] && $oConfig->Get('calendar', 'allow_sync', false)) {
					...
					$aData['Password'] = empty($aData['Password']) ? '' : static::APP_DUMMY;
					$aData['Interval'] = \max(20, \min(320, (int) $oConfig->Get('calendar', 'sync_interval', 20)));
					unset($aData['PasswordHMAC']);
					$aResult['CalendarSync'] = $aData;
				}

				$sToken = \Tachyon\Util\Cookies::get(self::AUTH_MAILTO_TOKEN_KEY);
				if (null !== $sToken) {
					\Tachyon\Util\Cookies::clear(self::AUTH_MAILTO_TOKEN_KEY);

					$mMailToData = Utils::DecodeKeyValuesQ($sToken);
					if (!empty($mMailToData['MailTo']) && 'MailTo' === $mMailToData['MailTo'] && !empty($mMailToData['To'])) {
						$aResult['mailToEmail'] = \Tachyon\Util\IDN::emailToUtf8($mMailToData['To']);
					}
				}
...
//				}
	...
	public function Upload(?array $aFile, int $iError): array
	{
		...
		if ($oAccount && UPLOAD_ERR_OK === $iError && \is_array($aFile)) {
			...
			// Detect content-type
			$type = \Tachyon\Util\File\MimeType::fromFile($aFile['tmp_name'], $aFile['name'])
				?: \Tachyon\Util\File\MimeType::fromFilename($aFile['name']);
			if ($type) {
				$aFile['type'] = $type;
				$sSavedName .= \Tachyon\Util\File\MimeType::toExtension($type);
			}
	...
	public function Capa(bool $bAdmin, ?Model\Account $oAccount = null): array
	{
		...
		if (!$aResult) {
			...
			$aResult = array(
				...
				Capa::SIEVE               => false,
				Capa::ACCOUNT_SEARCH      => (bool) $oConfig->Get('imap', 'search_all_folders', true),
