...
//		return $this->DefaultResponse(false);
	...
	public function DoAdminPasswordUpdate() : array
	{
		...
		$oNewPassword = new \Tachyon\Util\SensitiveString($this->GetActionParam('newPassword', ''));
...
//			"otpauth://totp/{$user}?secret={$secret}",
	...
	private function setAdminAuthToken() : string
	{
		...
		\Tachyon\Util\Cookies::set(static::$AUTH_ADMIN_TOKEN_KEY, $sToken);
		return $sToken;
	}
	...
	public static function AdminAppData(Actions $oActions, array &$aResult): void
	{
		...
		if ($aResult['Auth']) {
			...
			$aResult['supportedPdoDrivers'] = \Tachyon\Pdo\Base::getAvailableDrivers();
			...
			$aResult['Admin']['languages'] = \Tachyon\Util\L10n::getLanguages(true);
