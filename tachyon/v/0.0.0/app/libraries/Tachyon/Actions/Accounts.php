...
use Tachyon\Providers\Identities;
...
trait Accounts
{
	private ?\Tachyon\Providers\Identities $oIdentitiesProvider = null;
	...
	public function GetAccounts(MainAccount $oAccount): array
	{
		if ($this->GetCapa(Capa::ADDITIONAL_ACCOUNTS)) {
			...
			$aAccounts = $sAccounts ? \json_decode($sAccounts, true)
				: \Tachyon\Util\Upgrade::ConvertInsecureAccounts($this, $oAccount);
...
//				$this->SetAdditionalAuthToken(null);
				\Tachyon\Util\Cookies::clear(self::AUTH_ADDITIONAL_TOKEN_KEY);
				$bReload = true;
			}
...
/*		// TODO: verify private key for certificate?
		if ($oIdentity->smimeCertificate && $oIdentity->smimeKey) {
			new \Tachyon\Util\SMime\Certificate($oIdentity->smimeCertificate, $oIdentity->smimeKey);
		}
*/
