...
class ChangePasswordFroxlorDriver
{
	...
	{
		return array(
			\Tachyon\Plugins\Property::NewInstance('froxlor_dsn')->SetLabel('Froxlor PDO dsn')
				->SetDefaultValue('mysql:host=localhost;dbname=froxlor;charset=utf8'),
			\Tachyon\Plugins\Property::NewInstance('froxlor_user')->SetLabel('User'),
			\Tachyon\Plugins\Property::NewInstance('froxlor_password')->SetLabel('Password')
				->SetType(\Tachyon\Enumerations\PluginPropertyType::PASSWORD),
			\Tachyon\Plugins\Property::NewInstance('froxlor_allowed_emails')->SetLabel('Allowed emails')
	...
	{
		if (!\Tachyon\Plugins\Helper::ValidateWildcardValues($oAccount->Email(), $this->oConfig->Get('plugin', 'froxlor_allowed_emails', ''))) {
			return false;
		}
