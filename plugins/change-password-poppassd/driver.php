...
class ChangePasswordPoppassdDriver extends \MailSo\Net\NetClient
{
	...
	{
		return array(
			\Tachyon\Plugins\Property::NewInstance('poppassd_host')->SetLabel('POPPASSD Host')
				->SetDefaultValue('127.0.0.1'),
			\Tachyon\Plugins\Property::NewInstance('poppassd_port')->SetLabel('POPPASSD Port')
				->SetType(\Tachyon\Enumerations\PluginPropertyType::INT)
				->SetDefaultValue(106),
			\Tachyon\Plugins\Property::NewInstance('poppassd_allowed_emails')->SetLabel('Allowed emails')
	...
	{
		if (!\Tachyon\Plugins\Helper::ValidateWildcardValues($oAccount->Email(), $this->oConfig->Get('plugin', 'poppassd_allowed_emails', ''))) {
			return false;
		}
