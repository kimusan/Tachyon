...
namespace Tachyon\Providers\AddressBook;

use
	Sabre\VObject\Component\VCard,
	Tachyon\Providers\AddressBook\Classes\Contact,
	Tachyon\Providers\AddressBook\Enumerations\PropertyType
;
	...
	public function __construct()
	{
		...
		if ('sqlite' === $oSettings->driver) {
			...
			if (!$oConfig->Get('contacts', 'sqlite_global', \is_file(APP_PRIVATE_DATA . '/AddressBook.sqlite'))) {
				...
				if ($oAccount) {
					$homedir = \Tachyon\Api::Actions()->StorageProvider()->GenerateFilePath(
						$oAccount,
...
//					}
	...
	public function Sync() : bool
	{
		...
		try {
			...
			\Tachyon\Util\Log::error('DAV', $e->getMessage());
//			$this->logException($oException);
		}
		if (!$oClient) {
			\Tachyon\Util\Log::warning('PdoAddressBook', 'Sync() invalid DavClient');
			return false;
		}
		...
		if (false === $aRemoteSyncData) {
			\Tachyon\Util\Log::info('PdoAddressBook', 'Sync() no data to sync');
			return false;
		}
		$time = \microtime(true) - $time;
		\Tachyon\Util\HTTP\Stream::JSON(['messsage'=>"Received ".\count($aRemoteSyncData)." remote contacts in {$time} seconds"]);
		\Tachyon\Util\Log::info('PdoAddressBook', \count($aRemoteSyncData) . ' remote contacts');

		$aLocalSyncData = $this->prepareDatabaseSyncData();
		\Tachyon\Util\Log::info('PdoAddressBook', \count($aLocalSyncData) . ' local contacts');
...
//		$this->oLogger->WriteDump($aLocalSyncData);
		...
		if ($bReadWrite) {
			...
			if ($iCount) {
				\Tachyon\Util\Log::info('PdoAddressBook', $iCount . ' remote contacts removed');
			}
		} else {
			\Tachyon\Util\Log::info('PdoAddressBook', 'Sync() is import only');
		}
		...
		if (\count($aIdsForDeletion)) {
			\Tachyon\Util\HTTP\Stream::JSON(['messsage'=>'Delete local ' . \implode(', ', $aIdsForDeletion)]);
			$this->DeleteContacts($aIdsForDeletion);
			\Tachyon\Util\Log::info('PdoAddressBook', \count($aIdsForDeletion) . ' local contacts removed');
			...
			foreach ($aLocalSyncData as $sKey => $aData) {
				if ((empty($aData['etag']) && !isset($aRemoteSyncData[$sKey])) // new
				 ...
				 || (!empty($aData['etag']) && isset($aRemoteSyncData[$sKey]) &&
					...
					\Tachyon\Util\HTTP\Stream::JSON(['messsage'=>"Update remote {$sKey}"]);
					...
					if ($oContact) {
						...
						\Tachyon\Util\Log::info('PdoAddressBook', "Update contact {$sKey} in DAV");
						...
						if ($oResponse) {
							...
							\Tachyon\Util\Log::warning('PdoAddressBook', "Update/create remote failed");
						}
					} else {
						\Tachyon\Util\Log::warning('PdoAddressBook', "Local contact {$sKey} not found");
					}
					unset($oContact);
				}
			}
		...
		foreach ($aRemoteSyncData as $sKey => $aData) {
			if (!isset($aLocalSyncData[$sKey]) // new
			 ...
			 || ($aLocalSyncData[$sKey]['etag'] !== $aData['etag'] && $aLocalSyncData[$sKey]['changed'] < $aData['changed'])
				...
				\Tachyon\Util\HTTP\Stream::JSON(['messsage'=>"Update local {$sKey}"]);
				...
				if ($oVCard instanceof VCard) {
					...
					if ($oContact) {
						\Tachyon\Util\Log::info('PdoAddressBook', "Update local contact {$sKey}");
					} else {
						\Tachyon\Util\Log::info('PdoAddressBook', "Create local contact {$sKey}");
	...
	{
		if (1 > $this->iUserID) {
			\Tachyon\Util\Log::warning('PdoAddressBook', 'Export() invalid $iUserID');
			return false;
		}
...
//					if ($mData && isset($mData['id_contact'], $mData['deleted']) && !$mData['deleted']) {
	...
	{
		if (1 > $this->iUserID) {
			\Tachyon\Util\Log::warning('PdoAddressBook', 'ContactSave() invalid $iUserID');
			return false;
		}
...
//		$oContact->REV = \time();
	...
	{
		if (1 > $this->iUserID) {
			\Tachyon\Util\Log::warning('PdoAddressBook', 'DeleteContacts() invalid $iUserID');
			return false;
		}
...
//							$oContact->vCard->REV = \gmdate('Ymd\\THis\\Z', $oContact->Changed);
			...
			// Build vCards using old Tachyon data (missing jCard)
						...
						foreach ($aFetch as $aItem) {
							if ($aItem && isset($aItem['id_prop'], $aItem['id_contact'], $aItem['prop_type'], $aItem['prop_value'])) {
								...
								if (0 < $iId) {
									if ($iPrevId != $iId) {
										if ($iPrevId) {
											$aVCards[$iPrevId]->UID = $sUid ?: \Tachyon\Util\UUID::generate();
