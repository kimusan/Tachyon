...
/*
		...
		if ($aAuth) {
			...
			$oAccount->setSmtpPass(new \Tachyon\Util\SensitiveString($aAuth['password']));
			...
			if ($oMessage) {
				...
				if (false !== $iMessageStreamSize) {
					...
					if (\strlen($sFolderFullName) && 0 < $iUid) {
						...
						{
							$this->Cacher($oAccount)->Set(\Tachyon\KeyPathHelper::ReadReceiptCache($oAccount->Email(), $sFolderFullName, $iUid), '1');
						}
					}
				}
			}
...
//			$accountHash = $aValues[3];
		...
		try
		{
			...
			if ($oMessage->smimeSigned && ($bAutoVerify || !$oMessage->smimeSigned['detached'])) try {
				...
				$result = (new \Tachyon\Util\SMime\OpenSSL(''))->verify($sBody, null, $bOpaque);
				if ($result) {
					if ($bOpaque) {
					...
					if ('winmail.dat' === \strtolower($oAttachment->FileName())) {
						...
						foreach ($oTNEF->getFiles() as $oFile) {
							if (\in_array($oFile->type, ['application/rtf', 'text/rtf'])) {
								$rtf = new \Tachyon\Util\Rtf\Document($oFile->content);
							...
							$this->MailClient()->MessageMimeStream(
								function($rResource, $sContentType, $sFileName, $sMimeIndex = '') use ($oAccount, $sTempName, $self, &$aAttachments, $mIndex) {
									if (\is_resource($rResource)) {
										...
										$sTempName .= \Tachyon\Util\File\MimeType::toExtension($sContentType);
...
//												'size' => 0
							...
							$sContentType = \Tachyon\Util\File\MimeType::fromStream($rResource, $sTempName)
								?: \Tachyon\Util\File\MimeType::fromFilename($sTempName)
								?: 'application/octet-stream'; // 'text/plain'
							$aAttachments[$mIndex] = [
//								'name' => '',
								'tempName' => $sTempName,
...
*/
		...
		$oPassphrase = new \Tachyon\Util\SensitiveString($this->GetActionParam('signPassphrase', ''));
...
*/
			if ($sCertificate && $sPrivateKey) {
				...
				$resource = $oBody->ToStream();
				\MailSo\Base\StreamFilters\LineEndings::appendTo($resource);
				$tmp = new \Tachyon\Util\File\Temporary('mimepart');
