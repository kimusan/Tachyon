...
use Tachyon\Providers\Suggestions;
use Tachyon\Utils;
...
trait User
{
	...
	public function DoLogin() : array
	{
		try {
			$oAccount = $this->LoginProcess(
				...
				new \Tachyon\Util\SensitiveString($this->GetActionParam('Password', ''))
			);
	...
	public function DoAppDelayStart() : array
	{
		...
		$sTimers = $this->StorageProvider()->Get(null,
			\Tachyon\Providers\Storage\Enumerations\StorageType::NOBODY, 'Cache/Timers', '');
