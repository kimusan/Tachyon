...
abstract class AbstractConfig implements \ArrayAccess, \JsonSerializable
{
	...
	{
		// $salt = \basename($this->sFile) not possible due to Tachyon\Plugins\Property
		if (!empty($this->aData[$sSection][$sName][0])) try {
			return \Tachyon\Util\Crypt::DecryptFromJSON($this->aData[$sSection][$sName][0], \APP_SALT);
		} catch (\Throwable $e) {
		}
		return $mDefault;
	}
	...
	{
		// $salt = \basename($this->sFile) not possible due to Tachyon\Plugins\Property
		$mParamValue = \Tachyon\Util\Crypt::EncryptToJSON($mParamValue, \APP_SALT);
	...
	public function Save() : bool
	{
		...
		if (\str_ends_with($this->sFile, '.json')) {
			$this->clearCache();
			\Tachyon\Utils::saveFile($this->sFile, \json_encode($this, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES));
			// Remove old .ini file
		...
		\Tachyon\Utils::saveFile($this->sFile,
