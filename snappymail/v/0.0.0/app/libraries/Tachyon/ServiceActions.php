...
//						$this->oActions->logMask($value);
		...
		{
			\Tachyon\Util\Log::warning('SERVICE', "{$oException}");
			if ($e = $oException->getPrevious()) {
				\Tachyon\Util\Log::warning('SERVICE', "- {$e}");
			}
...
//				\is_uploaded_file($_FILES[$sInputName]['tmp_name'])
		...
		if ($sData
		  ...
		  && $this->oActions->getAccountFromToken()
			...
			if (!empty($sUrl)) {
				...
				if ($oResponse) {
					$sContentType = \Tachyon\Util\File\MimeType::fromStream($tmp) ?: $oResponse->getHeader('content-type');
					if (200 === $oResponse->status && \str_starts_with($sContentType, 'image/')) {
						try {
							...
							\Tachyon\Util\Log::error('Proxy', \get_class($HTTP) . ': ' . $e->getMessage());
						}
					} else {
	...
	{
		\Tachyon\Util\HTTP\CSP::logReport();
	}
...
//					error_log(print_r($e,1));
	...
	/**
	 * Login with the \Tachyon\API::CreateUserSsoHash() generated hash
	 */
	public function ServiceSso() : string
	{
		...
		if (!empty($sSsoHash)) {
			...
			if (!empty($sSsoSubData)) {
				$aData = \Tachyon\Util\Crypt::DecryptFromJSON($sSsoSubData, $sSsoHash);
					...
					(0 === $aData['Time'] || \time() - 10 < $aData['Time']))
				{
					...
					try
					{
						$oAccount = $this->oActions->LoginProcess(
							\trim($aData['Email']),
							new \Tachyon\Util\SensitiveString($aData['Password'])
						);
