...
namespace Tachyon\Util\SMime;

use Tachyon\Util\File\Temporary;
...
//						'validTo' => \gmdate('Y-m-d\\TH:i:s\\Z', $data['validTo_time_t']),
					...
					\Tachyon\Util\Log::error('OpenSSL', "parse({$file}): " . \openssl_error_string());
				}
			}
...
//		\glob("{$this->homedir}/*.key");
	...
	public function setPrivateKey(/*OpenSSLAsymmetricKey|string*/$privateKey,
		?\Tachyon\Util\SensitiveString $passphrase = null
	) : void
	{
	...
	public function exportPrivateKey(?\Tachyon\Util\SensitiveString $passphrase = null): string
	{
		if (!$this->privateKey) {
